...
function generate_login_key(serverseed, username, password)
{
	...
	if (xtest)
	{
		prefix = Buffer.from('2113b70280ad6e9a', 'hex')
	}
	var length = 8 + 4 + str.length
	var pad = Buffer.from(str_repeat(" ", (8 - length % 8) % 8), 'utf8')
	var strlen = Buffer.alloc(4)
	strlen.writeUInt32BE(str.length,0)
	var plain = Buffer.concat([prefix, strlen, Buffer.from(str, 'utf8'), pad])
	if (test)
	{
	...
	if (test)
	{
		...
		console.log('0f8596dd08fd009cf7f4c33bb4411c16cadea63e7b16ca7b expected' )
	}
...
function aochat_crypt(key, str)
{
	...
	var now = [0, 0]
	var prev = [0, 0]
	var ret = Buffer.alloc(str.length);
